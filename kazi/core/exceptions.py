...
class GuardrailViolationError(SecurityError):
    """LLM output was blocked by a GuardrailConfig rule (on_violation='block')."""


# ── Cost budget ───────────────────────────────────────────────────────────────

class BudgetExceededError(KaziError):
    """A run was blocked because it would exceed a configured cost budget."""


# ── Prompt injection ──────────────────────────────────────────────────────────

class InjectionDetectedError(SecurityError):
    """Incoming message matched a prompt-injection pattern (mode='block')."""
