...
class SecretRef:
    ...
    def __init__(self, value: str | Callable[[], str | None] | None = None) -> None:
        self._value = value

    # ── factories ─────────────────────────────────────────────────────────

    @classmethod
    def from_env(cls, var_name: str, fallback: str | None = None) -> SecretRef:
        """Resolve from an environment variable at call time."""
        return cls(lambda: os.environ.get(var_name, fallback))

    @classmethod
    def from_callable(cls, fn: Callable[[], str | None]) -> SecretRef:
        """Resolve by calling `fn()` each time — use for vault / secrets-manager."""
        return cls(fn)

    @classmethod
    def coerce(cls, value: str | SecretRef | None) -> SecretRef | None:
        """
